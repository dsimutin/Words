...
function selectedPalette(bestDays,fallback){const id=localStorage.getItem('wordsPalette:'+TOKEN),unlocked=STREAK_STATUSES.find(x=>x.id===id&&bestDays>=x.days);return unlocked||fallback}
function paletteSetting(d){const bestDays=Math.max(0,Number(d.longestStreak||0)),current=getStreakStatus(bestDays).current||STREAK_STATUSES[0],selected=selectedPalette(bestDays,current),options=STREAK_STATUSES.filter(x=>bestDays>=x.days).map(x=>`<option value="${x.id}" ${x.id===selected.id?'selected':''}>${esc(x.name)} · ${x.days} ${dayWord(x.days)}</option>`).join('');return`<label>Цветовая палитра<select onchange="setAchievementPalette(this.value)">${options}</select><small class="setting-note">Доступны палитры полученных медалей</small></label>`}
function setAchievementPalette(id){const bestDays=Math.max(0,Number(homeData&&homeData.longestStreak||0)),status=STREAK_STATUSES.find(x=>x.id===id&&bestDays>=x.days);if(!status)return;localStorage.setItem('wordsPalette:'+TOKEN,status.id);renderHome(homeData);toggleSettings()}
function achievementHeader(d){
 const streak=getStreakStatus(d.streak),best=getStreakStatus(d.longestStreak),current=best.current||STREAK_STATUSES[0],bestDays=Math.max(0,Number(d.longestStreak||0)),palette=selectedPalette(bestDays,current),lessons=Number(d.totalLessons||0),lessonDone=Number(d.today||0)>0,lessonStatusClass=lessonDone?'done':'pending',lessonStatusText=lessonDone?'Пройдено сегодня':'Урок сегодня не пройден',phrase=MOTIVATION_PHRASES[lessons%MOTIVATION_PHRASES.length],percent=Math.round(streak.progress*100),footer=streak.next?`До достижения «${streak.next.name}»: ${streak.days} из ${streak.next.days} дней · осталось ${streak.remaining} ${dayWord(streak.remaining)}`:'Все достижения серии открыты';
 app.style.setProperty('--achievement-accent',palette.color);
 app.style.setProperty('--green',palette.color);
