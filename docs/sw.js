...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith(caches.match(event.request).then(cached=>cached||caches.match(new URL(event.request.url).pathname.endsWith('/')?'./index.html':event.request).then(shell=>{
    const network=fetch(event.request).then(response=>{if(response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(event.request.mode==='navigate'?'./index.html':event.request,copy));}return response});
